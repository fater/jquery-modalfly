...
* **config** - задаем настройки плагина вторым аргументом функции `{url: 'путь к API', lang_save: 'Язык кнопки сохранить', lang_close: 'Язык кнопки закрыть'}`;
...
var param = {};
param.id = 74;
param.action = 'get_form';
param.param = 'user_info';
$.modalfly ('request', {url: 'http://yoursite.com/api.php', param: param});
```

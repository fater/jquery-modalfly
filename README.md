...
* Модуль может отправлять запросы на API адрес сервера и возвращать их в виде исполняемого JS кода или/и отображать полученные данные от сервера в модальном окне;
...
* **options** - задаем настройки плагина вторым аргументом функции `{url: 'путь к API', lang_save: 'Язык кнопки сохранить', lang_close: 'Язык кнопки закрыть'}`;
